...
class CapIQClient:
    ...
    # In case of an error, a None value is returned for that mnemonic and Cap
    # IQ's error is logged
    ...
    def get_cached_request_count(self):
        ...
        # if the file is not present then create one with todays date and a
        # count of 0
    ...
    def make_request(
            ...
            return_keys,
            properties,
            api_function_identifier,
            multiple_results_expected):
        req_array = []
        returnee = {}
        mnemonic_return_keys = self.build_mnemonic_return_key_index(
        ...
        for identifier in identifiers:
            for i, mnemonic in enumerate(mnemonics):
                req_array.append(
                    {"function": api_function_identifier,
                     "identifier": identifier, "mnemonic": mnemonic,
        ...
        if len(response.json()['GDSSDKResponse']) == 1 and \
            ...
            raise CiqServiceException(
                response.json()['GDSSDKResponse'][0]["ErrMsg"])
        for return_index, ret in enumerate(response.json()['GDSSDKResponse']):
            ...
            if ret['ErrMsg']:
                ...
                returnee[identifier][
                    self.get_return_key(
                        ret['Mnemonic'],
                        returned_properties, mnemonic_return_keys)] = None
            else:
                for i_m, h_m in enumerate(ret["Headers"]):
                    if multiple_results_expected:
                        returnee[identifier][self.get_return_key(
                            ret['Mnemonic'], returned_properties, mnemonic_return_keys)] = []
                        for row in ret["Rows"]:
                            returnee[identifier][
                                self.get_return_key(
                                    ret['Mnemonic'], returned_properties, mnemonic_return_keys)
    ...
    def build_mnemonic_return_key_index(mnemonics, return_keys, properties):
        ...
        for index, mnemonic in enumerate(mnemonics):
            if mnemonic in mnemonic_return_keys:
                mnemonic_return_keys[mnemonic].append(
                    {"key": return_keys[index], "properties": properties[index]})

            else:
                mnemonic_return_keys[mnemonic] = [
                    {"key": return_keys[index],
    ...
    def get_return_key(mnemonic, properties, mnemonics_to_return_key_index):
        ...
        if len(mnemonics_to_return_key_index[mnemonic]) == 1:
            ...
            for index, return_key in enumerate(
                    mnemonics_to_return_key_index[mnemonic]):
                match = True
                ...
                for property_name, property_value in properties.items():
                    if not (
                            ...
                            upper()
                            in
                            return_key["properties"]
                            and return_key["properties"][property_name.upper()] ==
