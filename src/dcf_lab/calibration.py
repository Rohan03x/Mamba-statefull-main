"""Calibration helpers for module signals.

Implements Platt scaling to map raw module scores onto calibrated probabilities
(or equivalently, back to the canonical ``[-1, 1]`` score range). The scaler is
fold-aware: serialize the fitted parameters per (module, horizon) pair and reload
at inference before signal weighting.
"""
...
class PlattScaler:
    ...
    The scaler fits parameters ``a`` and ``b`` such that

    .. math::
        P(y = 1 \mid s) = \sigma(a s + b)

    where ``s`` is the raw module score and ``\sigma`` is the logistic sigmoid.
    The calibrated score delivered to the rest of the pipeline is ``2P - 1`` to
    retain symmetry around zero.
    """
    ...
    def fit(self, x: Iterable[float], y: Iterable[int | float]) -> "PlattScaler":
        ...
        Args:
            x: Raw module scores (already standardized to ``[-1, 1]``).
            y: Binary realizations, typically ``1`` when the horizon return is
        ...
        # Guard against degenerate labels – fall back to identity mapping.
        unique_labels = np.unique(y_arr)
        if unique_labels.size < 2:
            self.params = PlattParams(a=1.0, b=0.0)
            return self

        # Initialize parameters following Platt's suggestion.
        prior1 = np.sum(y_arr)
        ...
        for _ in range(self.max_iter):
            ...
            # Gradient (first derivatives) with L2 regularization.
            diff = p - y_arr
            ...
            det = h11 * h22 - h21 * h21
            if abs(det) < EPS:  # Poor conditioning – bail out to avoid NaNs.
                break
    ...
    def transform(
        ...
        Args:
            x: Raw module scores to calibrate.
            return_proba: When ``True``, emit probabilities in ``[0, 1]``;
                otherwise return scores remapped to ``[-1, 1]``.
        """
...
class CalibratorStore:
    ...
    def put(self, module: str, horizon: int, scaler: PlattScaler, *, fold_id: str | None = None) -> None:
    ...
    def get(self, module: str, horizon: int, *, fold_id: str | None = None) -> PlattScaler | None:
        self._ensure_fold(fold_id)
