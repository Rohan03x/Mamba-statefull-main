...
class GBMQuantileForecaster(QuantileForecaster):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> 'GBMQuantileForecaster':
        ...
        for q in self.config.quantiles:
            if LGBM_AVAILABLE:
                import os
                # PRODUCTION HEDGE-FUND CONFIGURATION
                # Exact configuration used in production quant systems
                # Optimized for high-alpha quantile forecasting with safety constraints
                params = {
                    'objective': 'quantile',
                    'metric': 'quantile',
                    'alpha': q,  # Quantile level (e.g., 0.05, 0.25, 0.50, 0.75, 0.95)
                    'boosting_type': 'gbdt',
                    'num_leaves': 16,  # Conservative to prevent overfitting
                    ...
                    'min_child_samples': 40,  # min_data_in_leaf - prevents overfitting on noise
                    'feature_fraction': 0.8,  # colsample_bytree - feature sampling per tree
                    'bagging_fraction': 0.8,  # subsample - row sampling
                    'bagging_freq': 5,  # Bagging every 5 iterations
                    'reg_alpha': 1.0,  # lambda_l1 - L1 regularization
                    'reg_lambda': 1.0,  # lambda_l2 - L2 regularization
                    'random_state': 42,
                    'verbose': -1,  # Suppress LightGBM warnings
                    'force_col_wise': True,  # Optimize for col-wise access
                }
                ...
                # Fallback to sklearn's GBM (matching hedge-fund philosophy)
                model = GradientBoostingRegressor(
                    loss='quantile',
                    alpha=q,
                    n_estimators=150,  # Match LightGBM
                    learning_rate=0.05,  # Slow learning for robustness
                    max_depth=3,  # Shallow trees
                    min_samples_leaf=40,  # Match min_child_samples
                    subsample=0.8,  # Match bagging_fraction
                    random_state=42
                )
...
def generate_driver_distributions(
    ...
    pred_features = pd.DataFrame({
        ...
        ] * forecast_horizon if all(
            col in hist_data.columns 
            for col in ['current_assets', 'current_liabilities']
        ) else [0.1] * forecast_horizon
    })
